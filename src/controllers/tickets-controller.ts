...
import ticketService, { ticketTypeParams } from "@/services/tickets-service";
...
export async function createTicketType(req: AuthenticatedRequest, res: Response) {
  const { isRemote, price, includesHotel, } = req.body as ticketTypeParams;
  const { userId } = req;
  try {
    const ticketTypes = await ticketService.createTicketType(userId, { isRemote, includesHotel, price });
    return res.status(httpStatus.CREATED).send(ticketTypes);
  } 
  catch (error) {
    return res.sendStatus(httpStatus.NOT_FOUND);
